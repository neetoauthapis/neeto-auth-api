...
`--subdomain` is the fourth global flag. It picks the workspace rather than the format; see [Authentication](/cli/authentication#working-with-multiple-workspaces).
...
Commands that only report an outcome, such as `users delete`, `products enable`, and `products disable`, print `{"message": "..."}` instead of `data`.

## Quiet

`--quiet` returns only the `data` payload, with no envelope and no breadcrumbs. For outcome-only commands such as `users delete` it prints `success`. This is useful in scripts:
